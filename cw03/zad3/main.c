...
int monitor_file(char* path, char* archives_directory, unsigned int seconds, unsigned int total_time){
...
int monitor_file_cp(char* path, char* archives_directory, unsigned int seconds, unsigned int total_time){
    unsigned int current_time = 0;
...
int main(int argc, char* argv[]){
    ...
    char file_list[256];
    if(argc != 6){
        printf("Incorrect number of parameters. Expected name of file with filenames to monitor (string), monitor duration (positive integer), mode (0 or 1), memory limit (positive integer) and cpu limit in seconds (positive integer)\n");
        return -1;
    }
    ...
    unsigned long memory, cpu;
    if(sscanf(argv[4], "%lu", &memory) == EOF){
        printf("Incorrect second argument. Expected positive integer (memory limit)\n");
        return -1;
    }
    if(sscanf(argv[5], "%lu", &cpu) == EOF){
        printf("Incorrect second argument. Expected positive integer (cpu limit)\n");
        return -1;
    }
    struct rlimit memory_limit;
    memory_limit.rlim_cur = memory;
    memory_limit.rlim_max = memory;
    struct rlimit cpu_limit;
    cpu_limit.rlim_cur = cpu;
    cpu_limit.rlim_max = cpu;
    ...
    while(fscanf(list,"%s %d", path, &seconds) > 0) {
        ...
        if(child_pid[i - 1] == 0) {
            ...
            int total_copies = 0;
            if(mode == 0) total_copies = monitor_file(path, archives_directory, seconds, total_time);
            else if(mode == 1) total_copies = monitor_file_cp(path, archives_directory, seconds, total_time);
            struct rusage self_stats, children_stats;
            getrusage(RUSAGE_SELF, &self_stats);
            getrusage(RUSAGE_CHILDREN, &children_stats);
            pid_t current_pid = getppid();
            printf("PID: %d\n", current_pid);
            printf("User time: %ld (seconds)      %ld (microseconds)\n", self_stats.ru_utime.tv_sec + children_stats.ru_utime.tv_sec, self_stats.ru_utime.tv_usec + children_stats.ru_utime.tv_usec);
            printf("System time: %ld (seconds)     %ld (microseconds)\n", self_stats.ru_stime.tv_sec + children_stats.ru_stime.tv_sec, self_stats.ru_stime.tv_usec + children_stats.ru_stime.tv_usec);
            exit(total_copies);
        }
    }
    for(i = 0; i < lines; i++){
